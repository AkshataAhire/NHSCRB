...
_OUTCOME_CUES = {
    ...
    r"\b(utili[sz]ation|throughput|admission(s)?|readmission(s)?|attendance(s)?|uptake)\b": "utilization",
    r"\b(length of stay|LOS|waiting time(s)?|time to|duration|turnaround time)\b": "time",
    # Clinical / detection / effectiveness
    r"\b(mortality|morbidity|complication(s)?|effectiveness|efficacy|detection rate(s)?|remission|healing|hysterectomy)\b": "clinical",
    # Safety
    r"\b(adverse event(s)?|safety|harms?)\b": "safety",
    # PROs / HRQoL
    r"\b(PROs?|PROMs?|HRQoL|quality of life|EQ-5D|HUI3)\b": "pro",
    # Workforce/retention
    r"\b(retention|turnover)\b": "impact"
}

def _find_cues(text: str, patterns: Dict[str, str], max_hits: int = 8) -> List[str]:
    if not text:
        return []
    hits = []
    low = text.lower()
...
def _normalize_outcomes(v: Any) -> List[str]:
    ...
    for t in norm:
        if "qaly" in t or "utility" in t or "quality-adjusted" in t:
            mapped.append("qaly")
        elif "cost" in t or "economic" in t or "budget" in t or "icer" in t or "roi" in t or "nmb" in t or "tariff" in t or "price" in t or "saving" in t:
            mapped.append("cost")
        elif "readmission" in t or "admission" in t or "utilization" in t or "throughput" in t or "use" in t or "attendance" in t or "uptake" in t:
            mapped.append("utilization")
        elif "length of stay" in t or "los" in t or re.search(r"\btime\b", t):
            mapped.append("time")
        elif "mortality" in t or "morbidity" in t or "clinical" in t or "effectiveness" in t or "efficacy" in t or "detection rate" in t or "remission" in t or "healing" in t:
            mapped.append("clinical")
        elif "safety" in t or "adverse" in t or "harm" in t:
            mapped.append("safety")
        elif "prom" in t or "pro" in t or "hrqol" in t or "quality of life" in t or "eq-5d" in t or "hui3" in t:
            mapped.append("pro")
        elif "impact" in t or "effect" in t or "outcome" in t or "retention" in t or "turnover" in t:
