...
def _retry_api(client, system_prompt, user_prompt, model, max_retries=3, backoff=2.0):
    ...
    Returns raw string response (or empty string on total failure).
    """
    last_err = None
    for attempt in range(1, max_retries + 1):
        try:
            ...
            if attempt < max_retries:
                ...
                return f'{{"include": false, "reason": "api_error: {str(e)}", "has_comparator": false, "detected_comparator": "Unknown", "has_primary_outcomes": false, "detected_outcomes": [], "confidence": 0.0}}'
